...
import com.esri.viewer.components.toc.tocClasses.TocLayerInfoItem;
...
public class TOC extends Tree
{
	...
	public function setLayerVisible( serviceName:String, layerId:uint, visible:Boolean ):void {
		var layer:ArcGISDynamicMapServiceLayer;
		for each ( var tocItem:TocMapLayerItem in _tocRoots ) {
			if ( tocItem.layer is ArcGISDynamicMapServiceLayer ) {
				layer = tocItem.layer as ArcGISDynamicMapServiceLayer;
				if ( layer.name == serviceName ) {
//				if ( layer.url.indexOf( serviceName ) > -1 ) {
					for each ( var child:TocLayerInfoItem in tocItem.children ) {
						setTocItemVisible( child, layerId, visible );
					}
					break;
				}				
			}
	...
	private function setTocItemVisible( item:TocItem, layerId:uint, visible:Boolean ):void {
		if ( item.children && item.children.length > 0 ) {
			...
			if ( item is TocLayerInfoItem ) {
				if ( TocLayerInfoItem( item ).layerInfo.layerId == layerId ) {
					TocLayerInfoItem( item ).visible = visible;
					return;
				}
			}
		}

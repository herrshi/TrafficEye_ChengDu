...
public class ConfigManager extends EventDispatcher
{
    ...
    private function configService_resultHandler(event:ResultEvent):void
    {
        try
        {
			...
			var serverIp:String = configXML.map.@serverip;
			if ( serverIp ) {
			...
			if (geometryService)
			{
				...
				if ( serverIp )
				{
					geometryServiceUrl = geometryServiceUrl.replace( "[serverip]", serverIp );
				}
            ...
            {
                extentParam = ViewerContainer.urlConfigParams.extent;
                initialExtent = extentParam.replace(/,/g, " ");
            }
			else if ( FlexGlobals.topLevelApplication.parameters.extent != null ) {
				extentParam = FlexGlobals.topLevelApplication.parameters.extent;
				initialExtent = extentParam.replace(/,/g, " ");
			}			
            if (initialExtent)
            {
			...
			var centerParam:String;
            if (ViewerContainer.urlConfigParams.center != null)
            {
                centerParam = ViewerContainer.urlConfigParams.center;
                mapCenter = centerParam.replace(/,/g, " ");
            }
			else if ( FlexGlobals.topLevelApplication.parameters.center != null ) {
				centerParam = FlexGlobals.topLevelApplication.parameters.center;
				mapCenter = centerParam.replace(/,/g, " ");
			}
            if (mapCenter)
            {
			...
			else if ( FlexGlobals.topLevelApplication.parameters.level != null ) {
				mapLevel = FlexGlobals.topLevelApplication.parameters.level;
			}
            if (mapLevel)
            {
                var levelObj:Object =
			...
			else if ( FlexGlobals.topLevelApplication.parameters.scale != null ) {
				mapScale = FlexGlobals.topLevelApplication.parameters.scale;
			}
            if (mapScale)
            {
                var scaleObj:Object =
...
//                    webMapUtil.arcgisSharingURL = arcgisSharingURL;
			...
			configData.showErrow = configXML.showerror == "true" ? true : false;
			
			
			//=================================================
			//WebService
			//================================================
			var serviceList:XMLList = configXML.webservices.webservice;
			for ( i = 0; i < serviceList.length(); i++ ) {
				var serviceName:String = serviceList[i].@name;
				var wsdl:String = serviceList[i].@wsdl;
				var service:String = serviceList[i].@service;
				var port:String = serviceList[i].@port;
				...
				configData.webservices.push( serviceObj );
			}
			...
			var remoteList:XMLList = configXML.remoteobjects.remoteobject;
			for ( i = 0; i < remoteList.length(); i++ ) {
				var name:String = remoteList[i].@name;
				var enable:Boolean = ( String( remoteList[i].@enable ) == "true" ) ? true : false;
				var destination:String = remoteList[i].@destination;
				var endPoint:String = remoteList[i].@endpoint || configXML.remoteobjects.@endpoint;
				var refreshRate:uint = remoteList[i].@refreshrate || 0;
				var remote:Object = {
					name: name,
					enable: enable,
				...
				configData.remoteObjects.push( remote );
			}

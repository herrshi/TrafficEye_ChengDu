...
package com.esri.viewer
{
	...
	public class BaseWidget extends Module implements IBaseWidget
	{
		...
		private var _widgetStateBeforeClose:String = WidgetStates.WIDGET_OPENED;
		...
		private function initWidgetTemplate(event:Event):void
		{
			...
			AppEvent.dispatch( AppEvent.WIDGET_CREATED, this.widgetId );
		}
		...
		public function getWidgetId( widgetName:String ):Number {
			for ( var i:uint = 0; i < _configData.widgets.length; i++ ) {
				if ( _configData.widgets[i].label == widgetName ) {
					return _configData.widgets[i].id;
				}
			}
			return -1;
		}
		...
		public function openWidget( widgetName:String ):void {
			var widgetId:int = getWidgetId( widgetName );
			if ( widgetId > -1 ) {

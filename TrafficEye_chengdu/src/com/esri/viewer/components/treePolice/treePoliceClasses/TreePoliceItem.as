package com.esri.viewer.components.treePolice.treePoliceClasses
{
	...
	public class TreePoliceItem extends EventDispatcher
	{
		public function TreePoliceItem( parentItem:TreePoliceItem = null ) {
			_parent = parentItem;
		}
		...
		private var _parent:TreePoliceItem;
		
		public function get parent():TreePoliceItem {
			return _parent;
		}
		 ...
		 */
		internal function addChild(item:TreePoliceItem):void {
			if (!children) {
		...
		internal function setVisible(value:Boolean):void
		{
			if (value != _visible || _indeterminate )
			{
				...
				for each ( var child:TreePoliceItem in children ) {
				...
				//通知地图处理状态改变
				if ( this is TreePolicePersonItem ) {
					var data:Object = {
						item: ( this as TreePolicePersonItem ).person.policeCode,
						visible: value
					}
					AppEvent.dispatch( AppEvent.POLICETREE_PERSON_CHECKBOX_CLICKED, data );
				}
				if ( this is TreePoliceTeamItem ) {
					AppEvent.dispatch( AppEvent.POLICETREE_TEAM_CHECKBOX_CLICKED, 
						{ id: ( this as TreePoliceTeamItem ).team.teamCode,
						   visible: value } );
				}
//					AppEvent.dispatch( new AppEvent( AppEvent.POLICETREE_CHECKBOX_CLICKED, { item:this, visible: value } ) );
		...
		internal function updateIndeterminateState(calledFromChild:Boolean = false):void
		{
			...
			for each (var item:TreePoliceItem in children)
			{
